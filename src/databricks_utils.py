import os, logging, pandas as pd, sys, re, uuid, pyspark.pandas as ps, duckdb
...
def create_df_from_source(source_type, table_name, settings):
    if source_type == "local_csv":
        ...
        for col in df.columns:
            df = df.withColumnRenamed(col, re.sub(r'[^a-zA-Z0-9]', '_', col)) #clean columns
...
def write_df_to_excel(source, sheet_name, file_path):
    """Writes the output of an in-memory spark df to an Excel sheet.""" #fuck
    if args.platform == "duckdb_on_databricks":
        df_pandas = duckdb.sql(f"SELECT * FROM {source}").df()
    ...
    df_pandas = df_pandas.drop(columns=[col for col in remove_cols if col in df_pandas.columns])
    ...
    if os.path.exists(local_tmp_path):
        with pd.ExcelWriter(local_tmp_path, engine='openpyxl', mode='a', if_sheet_exists="replace") as writer:
            df_pandas.to_excel(writer, sheet_name=sheet_name[:31], index=False)
            # print("mode append")
    else:
...
def create_exclusive_diff_tables(info):
    spark.sql(f"""
        ...
        SELECT {info['source1_prefixed_string']} FROM source1_{session_guid} source1
        LEFT JOIN source2_{session_guid} source2 ON source1.pk_hash = source2.pk_hash
        WHERE source2.pk_hash IS NULL
    """)
 
    spark.sql(f"""
        CREATE TABLE source2_minus_source1_{session_guid} AS
        SELECT {info['source2_prefixed_string']} FROM source2_{session_guid} source2
        LEFT JOIN source1_{session_guid} source1 ON source2.pk_hash = source1.pk_hash
...
def tag_exact_row_matches():
    ...
    spark.sql(f"""
        ...
        ON COALESCE(source1.pk_hash,'') = COALESCE(source2.pk_hash,'')
        AND COALESCE(source1.non_pk_hash,'') = COALESCE(source2.non_pk_hash,'')
    """)
    ...
    spark.sql(f"""
        ...
        FROM (
            ...
            WHERE source1.matched = False
              ...
              AND COALESCE(source1.non_pk_hash,'') = COALESCE(source2.non_pk_hash,'') --will attempt recon only if non_pk_hashes are same
        )
        WHERE GREATEST(
