...
class Recon:
    ...
    def run(self, recon_type):
        ...
        #     full_file_path = os.path.join(self.file_write_path, 'recon.log')
        #     # Wait for file flush if needed
        #     wait_time = 5
        #     while not os.path.exists(local_path) and wait_time > 0:
        #         time.sleep(1)
        #         wait_time -= 1
        ...
        #     # Now copy using dbfs:/ source path
        #     dbutils.fs.cp(local_path, full_file_path)       
    ...
    def write_recon_with_pk_results(self):
        ...
        for table in ["source1_minus_source2", "source2_minus_source1", "column_mismatches"]:
            ...
            (table, self.file_write_path, self.recon_scenario, self.info["mapping_df"])
        if args.platform in ["databricks","duckdb_on_databricks"]:
